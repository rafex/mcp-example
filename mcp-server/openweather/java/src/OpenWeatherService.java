...
public final class OpenWeatherService {
    ...
    public static String buildToolsPayload() {
        return "{"
            + "\"tools\":["
            + "{"
            ...
            + "\"inputSchema\":{\"type\":\"object\",\"properties\":{\"query\":{\"type\":\"string\",\"description\":\"Ciudad o ciudad,country code. Ejemplo: London,uk\"},\"units\":{\"type\":\"string\",\"description\":\"standard, metric o imperial\"},\"date\":{\"type\":\"string\",\"description\":\"Fecha opcional YYYY-MM-DD\"}},\"required\":[\"query\"],\"additionalProperties\":false}"
            + "}"
            + "]"
            + "}";
    }
    ...
    public static String fetchWeatherOverview(String query, String units, String date) {
        ...
        return mergeResolvedLocation(response, coordinates, query, normalizeUnits(units));
    }

    public static String buildResourcesPayload() {
        return toJsonValue(Map.of(
            "resources",
            List.of(
                resource("openweather://service-overview", "service-overview", "Resumen del MCP OpenWeather directo contra la API pública."),
                resource("openweather://unit-reference", "unit-reference", "Referencia de unidades soportadas por OpenWeatherMap.")
            )
        ));
    }

    public static String buildResourceContentsPayload(String resourceUri) {
    ...
    public static String buildPromptsPayload() {
        return toJsonValue(Map.of(
            "prompts",
            List.of(
                prompt(
    ...
    public static String buildPromptDetailsPayload(String promptName, String query, String units) {
        ...
        if ("current-weather-brief".equals(promptName)) {
            return toJsonValue(Map.of(
                ...
                "messages", List.of(
                    message("system", "Usa get_current_weather y resume el resultado sin inventar datos."),
        ...
        if ("weather-overview-brief".equals(promptName)) {
            return toJsonValue(Map.of(
                ...
                "messages", List.of(
                    message("system", "Usa get_weather_overview y responde con el resumen devuelto."),
    ...
    private static String mergeResolvedLocation(String json, Coordinates coordinates, String query, String units) {
    ...
    private static String toJsonValue(Object value) {
        ...
        if (value instanceof Map<?, ?> map) {
            ...
            for (Map.Entry<?, ?> entry : map.entrySet()) {
                ...
                first = false;
                builder.append(toJsonValue(entry.getKey().toString()));
                builder.append(":");
